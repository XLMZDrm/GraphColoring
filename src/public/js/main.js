let generateDinh = () => {
  ...
  for (var i = 1, j = 0; i <= dinh && j < dinh; i++, j++) {
    elements += "<h3>Đỉnh " + i + "</h3><br>" + '<label for="' + j + '">Các đỉnh nối với đỉnh ' + i + "</label><br>" + '<input type="text" name="' + j + '"></input><br>';
  }
  elements += '<label for="algorithm">Thuật toán: </label>' + '<select name="algorithm"><option value="greedy" selected>Greedy</option><option value="welch">Welch Powell</option><option value="byLevel">Tô màu theo bậc</option></select><br>';
